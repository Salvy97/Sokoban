...
TimeLimit::TimeLimit() : GameManager(), timeLimit(0) { }

bool TimeLimit::levelManager(ALLEGRO_DISPLAY * display)
{
	if (!resetPressed)	
		setBitmapsAndMusic();
	...
	{		
		...
		do 
		{
			...
			{
				timeLimit -= 1 / constants.FPS;
				if (timeLimit <= 0)
				{
					updateGUI(timeLimit);
					al_draw_text(messagesFont, al_map_rgb(255, 255, 0), constants.messagesTextX, constants.messagesTextY, ALLEGRO_ALIGN_CENTRE, constants.youLostText);
					al_flip_display();			
					al_rest(4);
					returnToMenu = true;
					return true;
...
void TimeLimit::updateGUI(float GUITimer)
{
